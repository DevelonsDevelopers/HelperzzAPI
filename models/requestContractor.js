...
module.exports = class RequestContractor {
    ...
    static fetchRequests(request){
        return database.query('SELECT contractor_details.*, contractors.phone, contractors.image FROM request_contractors INNER JOIN contractors ON contractors.id = request_contractors.contractor INNER JOIN contractor_details ON contractor_details.contractor = contractors.id WHERE request = ?', [request])
    }
